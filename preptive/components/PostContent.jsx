...
const renderTextWithLinks = (text, language, isInTableCell = false) => {
  if (!text) return null;
...
export default function PostContent({ content, language }) {
  ...
  const renderContent = (item, index) => {
    switch (item.type) {
      case 'heading':
        ...
        const headingContent = renderTextWithLinks(item.text, language);
      ...
      case 'paragraph':
        const paragraphContent = renderTextWithLinks(item.text, language);
        ...
        return (
          <ul key={index} className="mb-6 space-y-2 pl-6">
            {item.items.map((listItem, idx) => {
              const listItemContent = renderTextWithLinks(listItem, language);
              ...
              <thead className="bg-gray-50">
                <tr>
                  {item.headers?.map((header, idx) => {
                    const headerContent = renderTextWithLinks(header, language, true);
                ...
                {item.rows.map((row, rowIdx) => (
                  <tr key={rowIdx} className={rowIdx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    {row.map((cell, cellIdx) => {
                      const cellContent = renderTextWithLinks(cell, language, true);
      ...
      case 'text':
        ...
        const textContent = renderTextWithLinks(item.text, language);
      ...
      case 'faq':
        ...
        const questionContent = renderTextWithLinks(item.question, language);
  ...
  return (
    <div className={`${language === 'hi' ? 'font-noto-devanagari' : 'font-inter'}`}>
